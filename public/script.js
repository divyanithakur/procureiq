...
function openOpportunityFromQuery() {
    const transactionId = Number(new URLSearchParams(window.location.search).get("transactionId") || 0);
    if (!transactionId) return false;
    const item = (window.procurementOpportunities || []).find(opportunity => Number(opportunity.id) === transactionId);
    if (!item) return false;
    openOpportunityDrawer(item);
    return true;
}
window.procureIQOpenOpportunityFromQuery = openOpportunityFromQuery;
...
window.addEventListener("procureiq:data-updated", () => {
    openOpportunityFromQuery();
});

if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => setTimeout(openOpportunityFromQuery, 300), { once: true });
} else {
    setTimeout(openOpportunityFromQuery, 300);
}
...
function renderFilteredOpportunities() {
    ...
    list.innerHTML = visible.map((item, index) => {
        ...
        <article class="opportunity opportunity-compact-row">
          <div class="opportunity-primary-row">
            ...
            <a class="opportunity-review-btn primary-cta"
              href="/workspace/act?transactionId=${encodeURIComponent(Number(item.id) || 0)}"
    ...
    // Investigate is intentionally a real document link.
    // The destination is the Exception Resolver with the exact transaction id.
    // This avoids fragile client-side click interception; the resolver then
    // opens the matching opportunity from the query string.
...
/* Investigate clicks are handled by the capture listener near the global state. */
...
async function getAIInsight(transactionId, id, button) {
    ...
    box.innerHTML = `<div class="ai-insight-header">Procurement explanation</div><p class="ai-loading">Reviewing verified procurement evidence...</p>`;

    try {
        const response = await procureiqApiFetch("/api/insight", {
            method: "POST",
            headers: { "Content-Type": "application/json", "Accept": "application/json" },
        ...
        box.innerHTML = `
            <div class="ai-insight-header">${result.fallback ? "Evidence-based procurement explanation" : "Procurement explanation"}</div>
            ${result.notice ? `<p class="ai-fallback-note">${escapeHTML(result.notice)}</p>` : ""}
            <div class="ai-insight-content">${formatAIResponse(result.insight)}</div>
        `;
    } catch (error) {
        ...
        box.innerHTML = `
            ...
            <p>${escapeHTML(error?.message || "Unable to generate AI insight.")}</p>
            <button type="button" class="primary-cta retry-ai" data-transaction-id="${transactionId}" data-target="${escapeHTML(id)}">Try Again</button>
        `;
    } finally {
        if (button) {
...
console.info("ProcureIQ build V55 loaded");
        setupUploadUX();
...
(function setupRazorpayCheckout() {
    ...
    const defaults = { IN:"INR", US:"USD", GB:"GBP", AE:"AED", SG:"SGD", DE:"EUR", FR:"EUR", AU:"USD", CA:"USD" };
    const planDetails = {
        starter:{label:"STARTER",title:"Starter · focused procurement intelligence",why:"For focused analysis",benefits:["5,000 stored transactions","50,000 AI tokens/day","Exception Resolver and PDF reporting"]},
        business:{label:"BUSINESS · POPULAR",title:"Business · deeper procurement control",why:"For teams running review cycles",benefits:["25,000 stored transactions","Contract Recovery, Guided Buying and Supplier Risk","200,000 AI tokens/day"]},
        pro:{label:"PRO",title:"Pro · higher-volume procurement intelligence",why:"For larger operating workflows",benefits:["100,000 stored transactions","Up to 15 users","Priority support and API-ready foundation"]}
    };
    ...
    function updatePricing(){
        ...
        if(currencySelect){ currencySelect.value=currency; currencySelect.disabled=countrySelect?.value === "IN"; }
        priceNodes.forEach(node=>{const plan=node.dataset.pricePlan; if(prices[currency]?.[plan]) node.innerHTML=`${symbols[currency]}${prices[currency][plan].toLocaleString("en-IN")} <small>/ month</small>`;});
        if(selectedPlan && planPrice) planPrice.textContent=`${symbols[currency]}${prices[currency][selectedPlan].toLocaleString("en-IN")}`;
    }
    ...
    async function startPayment(plan,clickedButton){
        ...
        const button=clickedButton||document.querySelector(`[data-razorpay-plan="${plan}"]`);if(button){button.disabled=true;button.dataset.originalText=button.innerHTML;button.innerHTML="Preparing payment…";}
        try{const currency=currentCurrency(),country=countrySelect?.value||"IN";setPaymentStatus("Creating a secure payment order…");const response=await procureiqApiFetch("/api/create-order",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify({plan,currency,country})});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||"Unable to create payment order.");if(!window.Razorpay)throw new Error("Razorpay Checkout failed to load. Refresh the page and try again.");
            const options={key:result.key_id,amount:result.amount,currency:result.currency,name:"ProcureIQ",description:result.plan_name,order_id:result.order_id,theme:{color:"#1f6feb"},handler:async payment=>{try{setPaymentStatus("Verifying payment securely…");const verify=await procureiqApiFetch("/api/verify-payment",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify(payment)});const v=await verify.json().catch(()=>({}));if(!verify.ok||!v.success)throw new Error(v.error||"Payment verification failed.");setPaymentStatus(`✓ Payment verified. ${String(v.plan||plan).toUpperCase()} is now active for 30 days.`);window.procureIQLoadAIUsage?.();}catch(error){setPaymentStatus(error.message||"Payment verification failed.",true);}},modal:{ondismiss:()=>setPaymentStatus("Payment window closed. No payment was confirmed.")}};
            const razorpay=new window.Razorpay(options);razorpay.on("payment.failed",e=>setPaymentStatus(e?.error?.description||"Payment failed. Please try again.",true));razorpay.open();
        }catch(error){setPaymentStatus(error.message||"Unable to start payment.",true);}finally{if(button){button.disabled=false;button.innerHTML=button.dataset.originalText||"Choose plan →";}}
    }
