...
(function setupRazorpayCheckout() {
    ...
    async function startPayment(plan,clickedButton){
        ...
        if(button){button.disabled=true;button.dataset.originalText=button.dataset.originalText||button.innerHTML;button.innerHTML="Preparing subscription…";}
        try{
            ...
            if(!result.subscription_id||!result.key_id)throw new Error("Razorpay subscription details were not returned. Please try again.");
            if(!window.Razorpay)throw new Error("Razorpay Checkout failed to load. Refresh the page and try again.");
            const options={key:result.key_id,subscription_id:result.subscription_id,name:"ProcureIQ",description:`${result.plan_name} subscription`,theme:{color:"#1f6feb"},handler:async payment=>{try{setPaymentStatus("Verifying subscription securely…");const verify=await procureiqApiFetch("/api/verify-subscription",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify(payment)});const v=await verify.json().catch(()=>({}));if(!verify.ok||!v.success)throw new Error(v.error||"Subscription verification failed.");setPaymentStatus(`✓ ${String(v.plan||plan).toUpperCase()} subscription is active.`);await loadBillingState();window.procureIQLoadAIUsage?.();}catch(error){setPaymentStatus(error.message||"Subscription verification failed.",true);}},modal:{ondismiss:()=>setPaymentStatus("Checkout closed. No subscription was confirmed.",true)}};
            const razorpay=new window.Razorpay(options);
            razorpay.on("payment.failed",e=>setPaymentStatus(e?.error?.description||"Payment failed. No subscription was activated.",true));
            // Close the plan-details modal only after Razorpay has been
            // initialized successfully. This keeps any API/configuration
            // error visible instead of making the checkout appear to do nothing.
            razorpay.open();
            closePlanModal();
        }catch(error){setPaymentStatus(error.message||"Unable to start subscription.",true);}
        finally{if(button){button.disabled=false;button.innerHTML=button.dataset.originalText||"Choose plan →";}}
    }
    ...
    continueButton?.addEventListener("click",()=>{const plan=selectedPlan;if(plan)startPayment(plan,document.querySelector(`[data-razorpay-plan="${plan}"]`));});
