...
app.post("/api/create-subscription", async (req, res) => {
  ...
  try {
    ...
    await ensureSubscriptionsTable();
    // Only an authenticated/active subscription should block a new checkout.
    // A Razorpay subscription in `created` state means the customer has not
    // completed the initial authorization yet. If checkout is closed, blocked,
    // or otherwise interrupted, keeping that local `created` row as a hard
    // lock makes every later click on "Start Payment" fail before Razorpay
    // Checkout can even open. Allow a fresh subscription attempt instead.
    const existing = await pool.query(`SELECT razorpay_subscription_id, status, cancel_at_cycle_end, plan, currency, amount FROM subscriptions WHERE clerk_user_id=$1 AND status IN ('authenticated','active') AND cancel_at_cycle_end=FALSE ORDER BY updated_at DESC LIMIT 1`, [userId]);
    if (existing.rowCount) {
      ...
      return res.status(409).json({ error: "An active subscription already exists on this account. Cancel it before starting another subscription." });
    }
